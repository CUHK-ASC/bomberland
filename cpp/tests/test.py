...
def comp(a, b) -> bool:
    ...
    if type(a) == str or type(a) == int:
        ...
        if len(a) != len(b):
            ...
            return False
        for x, y in zip(
            sorted(a, key=lambda x: x if type(x) != dict else x["x"] + x["y"] * 100),
            sorted(b, key=lambda x: x if type(x) != dict else x["x"] + x["y"] * 100),
        ):
            if not comp(x, y):
                print(f"{x} != {y}")
                return False
...
def test_log(file: str) -> bool:
    with open(file, "r") as cin:
        r = [
            x.split("|")
            for x in cin.read().split("\n")
            if len(x.split("|")) > 2 and x.split("|")[0].find("agent-") >= 0
        ]
    r = [x for x in r if x[1].find("TEST") >= 0]

    a = sorted(
        [[int(x[2]), x[3]] for x in r if x[0].find("agent-a") > 0], key=lambda x: x[0]
    )
    b = sorted(
        [[int(x[2]), x[3]] for x in r if x[0].find("agent-b") > 0], key=lambda x: x[0]
    )
...
test_files = 0
for f in files:
    if not f.endswith(log_file_ends):
        continue
